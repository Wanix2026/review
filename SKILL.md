...
1. **小节拆分**：大纲每个子项独立为 `###` 小节，不同知识点绝不压进同一个 h3
2. **表格优先**：对比内容（两种脱水的区别）、数值参数（正常值/分类标准）、分类体系 → **必须**用表格，不用段落
3. **数值全加粗**：每个精确数值（温度、时间、剂量、正常值范围、天数、百分比）用 `**数值**` 加粗标记
4. **术语全加粗**：每个医学核心术语首次出现时用 `**术语**` 加粗
5. **步骤逐条列**：操作步骤、处理原则用 `1. 2. 3.` 逐条列出，不合并不概括
6. **章节末尾加权重标注**：多学时+多重点章末尾加 `> 本章权重：X学时，大纲重点Y项`
